  ...
  container.innerHTML = `
    <div class="dashboard">
      <div class="header">
        ...
        <div class="header__meta">${new Date().toLocaleDateString(undefined, { dateStyle: "medium" })}</div>
      </div>
...
function buildKpiCard(kpi: KPI, index: number): string {
  ...
  if (kpi.change !== undefined) {
    ...
    return `
      <div class="card kpi ${cls}" style="animation-delay: ${index * 0.08 + 0.05}s">
  ...
  return `
    <div class="card kpi card--accent" style="animation-delay: ${index * 0.08 + 0.05}s">
      <span class="kpi__label">${escapeHtml(kpi.label)}</span>
      <span class="kpi__value">${val}</span>
    </div>
  `;
