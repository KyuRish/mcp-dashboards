...
export function createServer(): McpServer {
  ...
  registerAppTool(
    ...
    async (): Promise<CallToolResult> => {
      return await _buildChartResult(
        ...
        "Effects Catalog: 5 effect previews with the bundled treatments labeled. Click any card to use it. Pass effects=<name> on any chart to override theme defaults."
      );
    }
  );
  ...
  registerAppTool(
    ...
    async (args: { theme?: string }): Promise<CallToolResult> => {
      ...
      const charts: Array<{ type: string; title: string; data?: unknown; labels?: string[]; datasets?: Array<{ label: string; data: any[] }> }> = [
        {
          type: "bar",
          title: "Charts",
